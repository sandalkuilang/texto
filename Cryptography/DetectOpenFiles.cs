...
////    All rights reserved.
////    http://vmccontroller.codeplex.com/SourceControl/changeset/view/69631
...
namespace VmcController.Services
{
    ...
    public class DetectOpenFiles : ServicedComponent
    {
        ...
        private static readonly string[] handleTypeTokens = new string[] { "", "", "Directory", "SymbolicLink", "Token",
        ...
        /// <returns></returns>
        public static IEnumerator<Dictionary<IntPtr, FileInfo>> GetOpenFilesEnumerator(int processId)
        {
            return new OpenFiles(processId).GetEnumerator();
        }

        private sealed class OpenFiles : IEnumerable<Dictionary<IntPtr, FileInfo>>
        {
            ...
            public IEnumerator<Dictionary<IntPtr, FileInfo>> GetEnumerator()
            {
                NT_STATUS ret;
                int length = 0x10000;
                                        ...
                                        if (GetHandleType(handle, handleEntry.OwnerPid, out handleType) && handleType == SystemHandleType.OB_TYPE_FILE)
                                        {
                                            ...
                                            if (GetFileNameFromHandle(handle, handleEntry.OwnerPid, out devicePath))
                                            {
                                                ...
                                                if (ConvertDevicePathToDosPath(devicePath, out dosPath))
                                                {
                                                    ...
                                                    {
                                                        Dictionary<IntPtr, FileInfo> fileInfo = new Dictionary<IntPtr, FileInfo>();
                                                        fileInfo.Add(handle, new FileInfo(dosPath));
