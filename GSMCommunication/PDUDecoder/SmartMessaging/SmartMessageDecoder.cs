...
using GSMCommunication.PDUDecoder.SmartMessaging;
using System;
...
/// <para>To determine, whether a message is part of a concatenated message at all, use <see cref="M:GSMCommunication.PDUDecoder.SmartMessaging.SmartMessageDecoder.IsPartOfConcatMessage(GSMCommunication.PDUDecoder.SmsPdu)" />
/// When you have identified that a message is a part of a concatenated message, you need to find the other message parts
/// belonging to the same message, <see cref="M:GSMCommunication.PDUDecoder.SmartMessaging.SmartMessageDecoder.ArePartOfSameMessage(GSMCommunication.PDUDecoder.SmsPdu,GSMCommunication.PDUDecoder.SmsPdu)" /> can be used for this.
/// </para>
/// <para>After all parts of the concatenated message have been identified, the parts need to be combined. This is done
/// using <see cref="M:GSMCommunication.PDUDecoder.SmartMessaging.SmartMessageDecoder.CombineConcatMessage(System.Collections.Generic.IList{GSMCommunication.PDUDecoder.SmsPdu})" /> and <see cref="M:GSMCommunication.PDUDecoder.SmartMessaging.SmartMessageDecoder.CombineConcatMessageText(System.Collections.Generic.IList{GSMCommunication.PDUDecoder.SmsPdu})" />. The difference between these two
/// methods is, that the first one returns the combined data in binary format, whereas the latter returns the result
/// as text.</para>
/// <para>To verify that all message parts are present before attempting to combine them, <see cref="M:GSMCommunication.PDUDecoder.SmartMessaging.SmartMessageDecoder.AreAllConcatPartsPresent(System.Collections.Generic.IList{GSMCommunication.PDUDecoder.SmsPdu})" />
/// can be used. Calling this method is optional, but prevents exceptions when combining without all parts present.</para>
/// <para>The above methods all accept <see cref="T:GSMCommunication.PDUDecoder.SmsPdu" /> instances to abstract some of the work that has to be done
/// to find and combine concatenated SMS messages. However, it is also possible to retrieve the underlying data that
/// is used for these operations (and also possible for other operations as well) using the methods <see cref="M:GSMCommunication.PDUDecoder.SmartMessaging.SmartMessageDecoder.DecodeUserDataHeader(System.Byte[])" />
/// and <see cref="M:GSMCommunication.PDUDecoder.SmartMessaging.SmartMessageDecoder.GetConcatenationInfo(GSMCommunication.PDUDecoder.SmsPdu)" />.</para>
/// </remarks>
